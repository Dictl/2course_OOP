...
ostream& operator<<(ostream& stream, const Polynom& polynom) {
    ...
    if (!polynom.expanded){
        ...
        {
            polynom.roots->get_numbers()[i].get_image() == 0 ? brace = '\0' : brace = '(';
            if (polynom.roots->get_numbers()[i].get_real()<0) stream<<"(x" << "+" << brace << polynom.roots->get_numbers()[i]*-1<<")";
            else stream<<"(x"<< "-" << brace << polynom.roots->get_numbers()[i]<<")";
        }
        polynom.roots->get_numbers()[polynom.size-1].get_image() == 0 ? brace = '\0' : brace = '(';
        if (polynom.roots->get_numbers()[polynom.size-1].get_real()<0) stream<<"(x" << "+" << brace << polynom.roots->get_numbers()[polynom.size-1]*-1<<"))";
        else stream<<"(x"<< "-" << brace << polynom.roots->get_numbers()[polynom.size-1]<<")";
    }
    else {
        if (polynom.a_n.get_image() != (double)0) stream << "(";
        ...
        for (size_t i = 0; i<polynom.size; i++){
            ...
            (polynom.coefficients->get_numbers()[i].get_image() == 0) ? brace = '\0' : brace = '(';
            if (polynom.size-i!=1 && (polynom.a_n*polynom.coefficients->get_numbers()[i] != number(1))){
                if (polynom.a_n.get_real()*polynom.coefficients->get_numbers()[i].get_real() < 0){
                    stream << "-" << brace << polynom.a_n*polynom.coefficients->get_numbers()[i]*-1 << "x^" << (int) polynom.size-i;
                }
                else{
                    if (i==0){
                        stream << brace << polynom.a_n*polynom.coefficients->get_numbers()[i] << "x^" << (int) polynom.size-i;
                    }
                    else stream << "+" << brace << polynom.a_n*polynom.coefficients->get_numbers()[i] << "x^" << (int) polynom.size-i;
                }
            }
                ...
                if (polynom.a_n.get_real()*polynom.coefficients->get_numbers()[i].get_real()<0){
                    stream << "-" << brace << polynom.a_n*polynom.coefficients->get_numbers()[i]*-1 << "x";
                }
                else{
                    if (i == 0){
                        stream << brace << polynom.a_n*polynom.coefficients->get_numbers()[i] << "x";
                    }
                    else stream << "+" << brace << polynom.a_n*polynom.coefficients->get_numbers()[i] << "x";
                }
            }
            else {
                if (polynom.a_n*polynom.coefficients->get_numbers()[i].get_real()<0){
        ...
        (polynom.coefficients->get_numbers()[polynom.size].get_image() == 0) ? brace = '(' : brace = '(';
        if (polynom.a_n.get_real()*polynom.coefficients->get_numbers()[polynom.size].get_real() < 0){
            stream << "-" << brace << polynom.a_n*polynom.coefficients->get_numbers()[polynom.size]*-1<<endl;
        }
        else{
            stream << "+" << brace << polynom.a_n*polynom.coefficients->get_numbers()[polynom.size]<<endl;
        }
    }
    return stream;
}
...
void Polynom::count_coefficients(){
    ...
    for (size_t i = 1; i < this->size; i++){ // for every root except the first one
        ...
        for (size_t j = 0; j < this->size; j++){
            ...
            } // and so every bracket will continously muptiply by an x and every root except the 1st
        }
    }
}
